...
def main():
    ...
    last_scan = time.time()
    if not has_redis:
        sys.stderr.write(" Redis module isn't installed.\n")
        return 1
    ...
    while True:
        ...
        port = 6379
        tags = "cluster=%s port=%d" % ('devops', port)

        # connect to the instance and attempt to gather info
        r = redis.Redis(host="devops.qdgrks.0001.usw2.cache.amazonaws.com", port=port)
        info = r.info()
        for key in KEYS:
            if key in info:
